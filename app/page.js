...
function LoginPage({ onLogin }) {
  ...
  const quick = async (a, b) => { setErr(''); setLoading(true); await onLogin(a, b, setErr); setLoading(false) }
...
// STUDENT: WELCOME
// ═══════════════════════════════════════
function WelcomePage({ settings, completedIds, totalChapters }) {
      ...
      {viewing && (
        <Modal title={viewing.title} onClose={() => setViewing(null)}>
          ...
          <div className="modal-actions">
            <button className="btn btn-sm" onClick={e => { toggleFavorite(viewing.id) }} style={{ background: 'var(--orange-bg)', color: 'var(--orange-text)', border: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
...
export default function Home() {
  ...
  const loadData = useCallback(async () => {
    ...
    // Nest chapters inside parts
    const allChapters = chaptersRes.data || []
    const partsWithChapters = (partsRes.data || []).map(p => ({
      ...p,
      chapters: allChapters.filter(c => c.part_id === p.id)
    }))
    setParts(partsWithChapters)
    setLoading(false)
  }, [])

  // Load progress & favorites for student
  ...
  const login = async (username, password, setErr) => {
    ...
    else { setPage('welcome'); await loadStudentData(data.id) }
  }
  ...
  const logout = () => { setUser(null); setPage('welcome'); setCompletedIds([]); setFavoriteIds([]) }
  ...
  return (
    <div className="app-layout">
      ...
      <div className="main-content">
        {!isAdmin && page === 'welcome' && <WelcomePage settings={settings} completedIds={completedIds} totalChapters={totalChapters} />}
